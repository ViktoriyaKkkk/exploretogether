...
const Header = observer(() => {
	...
	return (<header>
			<nav
				...
				<button className='sm:hidden order-1 text-white p-2 ml-4' onClick={()=>setMenu(!menu)}><IconContext.Provider value={{ size: '2em' }}>
					{
						menu ? <HiX/> : <HiMenu/>
					}
...

				{
					...
					<div className={clsx('order-3 sm:order-2 h-screen text-3xl sm:text-base sm:h-fit w-full bg-black sm:bg-transparent ' +
						'sm:justify-between w-full flex sm:w-auto', !menu && 'hidden sm:flex')} id='mobile-menu-2'>
						<ul className='flex flex-col my-auto pb-14 sm:pb-0 sm:mt-0 space-y-14 sm:space-y-0 w-fit sm:mt-4 mx-auto font-medium sm:flex-row lg:space-x-3 lg:mt-0'>
							<Link to='/'
